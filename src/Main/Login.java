...
public class Login extends javax.swing.JFrame {
    ...
    public Login() {
        initComponents();
        setIcon();
        new FillCb().fillCombo("SELECT * FROM login ORDER BY username", combo_user, "username");
    }
    ...
    private void initComponents() {
        ...
        jLabel5.setText("Система розпізнавання");
        jPanel2.add(jLabel5, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 10, 300, 40));
        ...
        jButton2.setText("Забули пароль?");
        ...
        jCheckBox1.setText("Запам'ятати мене");
    ...
    public void login() {
        connect.connection();
        try {
            connect.executeSQL("SELECT * FROM login WHERE username='" + combo_user.getSelectedItem() + "'");
            connect.rs.first();
            if (connect.rs.getString("password").equals(txt_pass.getText())) {
                new Menu((String) combo_user.getSelectedItem()).setVisible(true);
                dispose();
            } else {
                message_txt.setText("Невірний пароль!");
                message_txt.setForeground(new Color(202, 66, 66));
            }
        } catch (NullPointerException | SQLException ex) {
            message_txt.setText("Користувача не існує!");
            message_txt.setForeground(new Color(202, 66, 66));
        }
        connect.disconnect();
    }
