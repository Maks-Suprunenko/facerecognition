...
public class RoundImage {
    ...
    public void roundPhoto(JFrame frame, String path, JLabel label, int round) {
        try {
            ImageIcon icon = new ImageIcon(new ImageIcon(path).getImage().getScaledInstance(label.getWidth() + 95, label.getHeight() + 95, Image.SCALE_SMOOTH));
            label.setIcon(new ImageIcon(RoundImage.getRoundImage(icon.getImage(), round)));
        } catch (IllegalArgumentException ex) {
            ex.printStackTrace();
            JOptionPane.showMessageDialog(null, "Фотографію не знайдено.");
